...
class AttributeValueDetails extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleAddUom(index) {
    let skuWithVariants = this.state.skuWithVariants;
    // alert(JSON.stringify(skuWithVariants));
  ...
  async saveProductVariant() {
    ...
    if (!this.state.productId) {
      this.setInputValue("responseMessage", "Product is empty");
      return;
    } 
    */
    if (!this.state.variantName) {
      ...
      this.state.variantAttributes.length > 0 &&
      this.checkAttributeNameAndValues(this.state.variantAttributes) === true
    ) {
      return;
    } else if (this.checkSKU(this.state.skuWithVariants) === true) {
      return;
    } else {
      ...
      const payLoad = {
        ...
        skuWithVariants: this.state.skuWithVariants,
      };
      console.log(payLoad);
      try {
                ...
                <td className="width75">
                  <div className="d-flex">
                    <input
                      ...
                      /*
                      onChange={(e) =>
                        this.setInputValue("variantName", e.target.value)
                      }
                      */
...
export default withRouter(AttributeValueDetails);
...
class AttributeValueDetails extends React.Component {
  constructor(props) {
    ...
    this.gwUrl = process.env.REACT_APP_SERVICE_API_GW_HOST;
    this.addAPI = {
      path: "maxisservice-inventory-service/endpoint/api/attribute-value/",
      headers: {
        "Content-Type": "application/json",
        token: "Bearer " + AuthUtil.getIdToken(),
        userid: AuthUtil.getUserId(),
    ...
    this.setInputValue = this.setInputValue.bind(this);
    this.saveAttributeValue = this.saveAttributeValue.bind(this);
  ...
  async getAllAttributes() {
    let attributeArray = [];
    try {
      let res = await fetch(this.gwUrl + this.getAllAttributesAPI.path, {
        method: "GET",
        headers: this.getAllAttributesAPI.headers,
      });
      let response = await res.json();
    ...
    this.setInputValue("attributes", attributeArray);
  }

  handleAttributeChange(selectedAttributeValue) {
    this.setInputValue("attributeId", selectedAttributeValue);
  }
  ...
  async saveAttributeValue() {
    if (!this.state.label) {
      this.setInputValue("responseMessage", "Name is empty");
      return;
    } else if (!this.state.attributeId) {
      this.setInputValue("responseMessage", "Attribute is empty");
      return;
    } else {
      this.setInputValue("responseMessage", "");
        ...
        <div>
          <table className="width100">
            <tbody className="width100">
              ...
              <tr className="width100">
                <td className="width25">
                  <label className="form-input-label2">Attribute</label>
                </td>
                <td>
                  <SelectSearch
              ...
              <tr>
                <td>
                  <Button
                    label="Save"
                    onClick={() => this.saveAttributeValue()}
                    className="p-button-raised p-button-info"
                  />
                </td>
              </tr>
