...
class ViewFormComponent extends React.Component {
	constructor(props) 
	{
		super(props);
	...
	async componentDidMount()
	{
		...
		if (this.state.roleName === "MAXISELLPADMIN" || this.state.roleName === "MAXISEFLPADMIN" || this.state.roleName === "MAXISELLPENTERPRISEADMIN")
		{
			...
			let eDropDownList = [];
			let eJson = await eResponse.json();
			let edata = eJson.result? eJson.result.response : [];
			for (let i = 0; i < edata.length; i++)
				eDropDownList.push({"value": edata[i].userId, "label": edata[i].name});
			meta["M_EL_LP_E"] = eDropDownList;
		}

		if (this.state.roleName === "MAXISELLPADMIN" || this.state.roleName === "MAXISEFLPADMIN" || this.state.roleName === "MAXISELLPSHIPMENTADMIN")
		{
			...
			let sdata = sJson.result? sJson.result.response : [];
			for (let i = 0; i < sdata.length; i++)
				sDropDownList.push({"value": sdata[i].userId, "label": sdata[i].name});
			meta["M_EL_LP_S"] = sDropDownList;
		}

		if (this.state.roleName === "MAXISELLPADMIN" || this.state.roleName === "MAXISEFLPADMIN" || this.state.roleName === "MAXISELLPWADMIN")
		{
			...
			let wdata = wJson.result? wJson.result.response : [];
		...
		let designResponse = await fetch	(	gwUrl + "onboarding-service/endpoint/design/detail", 
												{
												 ...
												 ,
												 "body"		: JSON.stringify({"designId": attemptDetails.result.designId})
					 							}
					 						)
		;
...
//		attemptDetails.result.designModel.steps = defaultSteps.steps;
		...
		let dropDownValues = [];
		for(let indexStep = 0; indexStep < designDetails.result.designModel.steps.length; indexStep++)
		{
			for(let indexProperty = 0; indexProperty < designDetails.result.designModel.steps[indexStep].property1.length; indexProperty++)
			{
				let propertyCode = designDetails.result.designModel.steps[indexStep].property1[indexProperty].code;

				for(let indexExtendedProperty = 0; indexExtendedProperty < attemptDetails.result.extendedPropertyList.propertyList.length; indexExtendedProperty++)
				{
					let extendedProperty = attemptDetails.result.extendedPropertyList.propertyList[indexExtendedProperty];
					
					if (propertyCode === extendedProperty.propertyCode)
					{
						 designDetails.result.designModel.steps[indexStep].property1[indexProperty].value = extendedProperty.propertyValue;
						 
						 if (designDetails.result.designModel.steps[indexStep].property1[indexProperty].type === "DROPDOWN")
						 	dropDownValues[designDetails.result.designModel.steps[indexStep].property1[indexProperty].code] = extendedProperty.propertyValue;
					}
				}
			}
		}
...
export default withRouter(ViewFormComponent);