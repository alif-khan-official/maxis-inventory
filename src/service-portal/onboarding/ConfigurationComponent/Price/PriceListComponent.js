...
import '../../../../App.css';
import PriceConfigureComponent from './PriceConfigureComponent';
class PriceListComponent extends React.Component {
    ...
    viewEntity(item, configs) {
        ...
        this.props.history.push({ "pathname": "/price-configure", "state": { "item": item, "item2": this.state.item, "configs": configs, "entityDetail": this.state.entityDetail, "mode": "VIEW" } });
        console.log("====item====");
        console.log(item);
    }

    addEntity(configs) {
        console.log("addEntity---> ", configs);
        this.props.history.push({ "pathname": "/price-configure", "state": { "item": [], "item2": this.state.item, "configs": configs, "entityDetail": this.state.entityDetail, "mode": "ADD" } });
    }

    editEntity(item, configs) {
        this.props.history.push({ "pathname": "/price-configure", "state": { "item": item, "item2": this.state.item, "configs": configs, "entityDetail": this.state.entityDetail, "mode": "EDIT" } });
    }
